...
export class TagBotFactory extends BotFactory {
  ...
  constructor(options = {}) {
    ...
    this.#announceAllowList = ('announceAllowList' in options)
      ...
      : []
    this.#hashtagBlocklist = ('hashtagBlocklist' in options)
      ? new Set(options['hashtagBlocklist'].map(name => this.#scrubName(name)))
      : new Set()
  }

  async canCreate (username) {
    return !this.#hashtagBlocklist.has(this.#scrubName(username))
  }

  async create (username) {
    if (!await this.canCreate(username)) {
      throw new Error(`Cannot create TagBot with username ${username}`)
    }
    const bot = new TagBot(this.#scrubName(username))
  ...
  async #handleCreate (activity) {
    ...
    for (const object of activity.object) {
      ...
      for (const tagName of tagNames) {
        if (!await this.canCreate(tagName)) {
          this._context.logger.warn(
            { object: object.id, tagName },
            'skipping unusable tagname'
          )
          continue
        }
        this._context.logger.debug(
